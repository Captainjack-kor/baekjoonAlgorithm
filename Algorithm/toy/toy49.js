...
[Advanced]
벨만-포드 알고리즘은 다익스트라 알고리즘과 마찬가지로 '한 정점으로부터 
각 정점 간 최단 거리(single-source shortest path)'를 구하는 알고리즘입니다.
각 알고리즘의 시간 복잡도는 O(V * LogV) (다익스트라, 힙 사용 시), O(V * E) (벨만-포드) 입니다.
따라서 다익스트라 알고리즘이 보다 효율적입니다. 하지만 다익스트라 알고리즘은 간선의 가중치가 음수인 경우에는 작동하지 않습니다.
